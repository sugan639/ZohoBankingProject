...
public class TransactionService {
	...
	public JSONObject fetchTransactions(JSONObject json, Role requesterRole, Long requesterId) throws TaskException {
		...
		if (!(hasTxnId || hasAccountFilter || hasCustomerFilter)) {
			throw new TaskException(
					"Bad Request: Provide transaction_id OR reference_number OR (account_number or customer_id) with from_date and to_date");
		}
		...
		if (hasCustomerFilter) {
			...
			for (Account acc : accounts) {
				List<Transaction> txns = transactionDAO.getFilteredTransactions(null, acc.getAccountNumber(), null,
			...
			List<Transaction> txns = transactionDAO.getFilteredTransactions(txnId, accNum,
					(requesterRole == Role.CUSTOMER ? requesterId : null), from, to, type, status, limit, offset);
